...
export const en: Translation = {
  ...
  // Contextual day-status headlines — headline + subtitle
  homeGreetingMorning: (name: string) => `Good morning, ${name}`,
  homeGreetingAfternoon: (name: string) => `Good afternoon, ${name}`,
  homeGreetingEvening: (name: string) => `Good evening, ${name}`,
  homeGreetingGeneric: (name: string) => `Hi, ${name}`,
  homeStatusReadyHeadline: (day: number, total: number) => `Day ${day}/${total}`,
  homeStatusReadySubtitle: (program: string) => `${program} — ready to train`,
  homeStatusResumeHeadline: (program: string) => `Finish ${program}`,
  homeStatusResumeSubtitle: (set: number, total: number) => `Set ${set}/${total}`,
  homeStatusResumeStaleSubtitle: 'Unfinished workout — check below',
  homeStatusResumeAndReadySubtitle: (otherProgram: string) => `Or start ${otherProgram}`,
  homeStatusTestReadyHeadline: 'Max test',
  homeStatusTestReadySubtitle: (program: string) => `${program} — ready`,
  homeStatusTestRestHeadline: 'Test after rest',
  homeStatusTestRestSubtitle: (program: string, when: string) => `${program} — ${when}`,
  homeStatusAllPausedSubtitle: 'Resume when you\'re ready',
  homeStatusSetupHeadline: 'Set up a program',
  homeStatusSetupSubtitle: 'Pick a level to get started',
  homeStatusSetupMixedHeadline: 'Resume or finish setup',

  // Quick CTA in status hero
  homeQuickCtaStart: (program: string, day: number) => `Start · ${program} · Day ${day}`,
  homeQuickCtaResume: (program: string, set: number, total: number) => `Finish · ${program} · Set ${set}/${total}`,
  homeQuickCtaTest: (program: string) => `Start test · ${program}`,
  homeQuickCtaSetup: (program: string) => `Set up · ${program}`,
  homeQuickCtaScroll: 'Show details',
  homeQuickCtaTrainAnyway: (program: string) => `Train anyway · ${program}`,

  // Adaptive 14-day goal
  homeGoalNin14: (n: number) =>
    n === 1 ? '1 workout in 14 days' : `${n} workouts in 14 days`,
  ...
  warmupGeneratorTitle: 'Warm-up generator',
  warmupGeneratorHint: 'Enter your working weight — we will build a warm-up ladder.',
  warmupGeneratorWorkingWeight: 'Working weight',
  warmupGeneratorBarWeight: 'Bar weight',
  warmupGeneratorPlateStep: 'Plate increment',
  warmupGeneratorGenerate: 'Generate',
  warmupGeneratorAddAll: 'Add all warm-up sets',
  warmupGeneratorAddSet: 'Use this set',
  warmupGeneratorAdded: 'Warm-up sets added',
  warmupGeneratorEmptyBar: 'Empty bar',
  warmupGeneratorPercent: (pct: number) => `${pct}% of working`,
  // RPE / RIR
  rpeLabel: 'RPE',
  ...
  rpeRirModeRir: 'RIR',
  rpeRirClear: 'Clear',
  rpeDescription: (v: number) => {
    if (v <= 3) return 'Very easy'
    if (v <= 5) return 'Easy'
    if (v <= 7) return 'Moderate'
    if (v === 8) return '2 reps in reserve'
    if (v === 9) return '1 rep in reserve'
    return 'Max effort'
  },
  rirDescription: (v: number) => {
    if (v === 0) return 'Nothing left'
    if (v === 1) return '1 rep in reserve'
    if (v === 2) return '2 reps in reserve'
    if (v <= 5) return `${v} reps in reserve`
    return 'Very easy'
  },
  setNoteLabel: 'Set note',
  ...
  dropsetWeightReduction: 'Weight reduction',
  // Estimated 1RM
  est1rmTitle: 'Estimated 1RM',
  est1rmHint: 'Based on recent sets (Epley/Brzycki formula)',
  est1rmExercise: 'Exercise',
  est1rmValue: 'Est. 1RM',
  est1rmReps: 'Reps',
  est1rmWeight: 'Weight',
  est1rmDate: 'Date',
  ...
  chartTestOverTime: 'Last set progress',
  progressLastSetChartHint: 'Reps from the last set in each session',
  progressLastSetChartAria: (count: number) => `Last set progress chart, ${count} sessions`,
  progressLastSetChartTooltip: 'Last set',
  progressProgramSectionTitle: (name: string) => name,
  progressScopeLabel: 'Scope',
  progressScopeActivity: 'Activity',
  progressScopePushups: 'Pushups',
  progressScopePullups: 'Pullups',
  ...
  workoutDayLabel: 'Day',
  workoutSetOfTotal: (current: number, total: number) => `Set ${current} of ${total}`,
  ...
  restReady: 'Ready!',
  restSetLabel: (current: number, total: number) => `Set ${current} of ${total}`,
  restLastSeconds: 'Rest ending soon',
  ...
  importBackupFileAria: 'Choose a backup file to import',
  ...
  pwaUpdateTitle: 'New app version',
  pwaUpdateBody: 'Load the latest changes and improvements.',
  pwaUpdateReload: 'Refresh',
  pwaUpdateLater: 'Later',
  pwaUpdateApplying: 'Updating…',
  pwaUpdateApplyingHint: 'Loading the new version. The page will refresh automatically.',
  ...
  progressActivityTitle: 'Activity',
  ...
  progressTotalRepsAllTime: 'All-time reps',
  progressVolumeTrend14d: '14d trend',
  ...
  themeDark: 'Dark',
  themeLight: 'Light',
  themeHint: '"System" follows your device settings.',
  highContrast: 'High contrast',
  highContrastHint: 'Improves text and edge readability.',
  trainingSettings: 'Training settings',
  weightUnitLabel: 'Weight unit',
  weightUnitKg: 'kg',
  weightUnitLb: 'lb',
  weightUnitHint: 'Affects weight display in workouts and plans.',
  ...
  profileDangerZoneHint: 'These actions are irreversible. Use with caution.',
  infoHintLabel: 'Show hint',
  profileDataImportHint: 'Restore a backup from a JSON file.',
  profileDataExportJsonHint: 'Full backup (progress, sessions, plans, exercises).',
  profileDataExportCsvHint: 'Export all sessions to a CSV file.',
  profileDataClearHint: 'Removes all local data from this device.',
  ...
  reminderHourHint: 'You get a notification at this time when a workout is available.',
  ...
  aiCoachSubsectionProvider: 'Provider',
  aiCoachSubsectionConnection: 'Connection',
  aiCoachSubsectionOptions: 'Options',
  aiCoachConnectionHint: 'The API key is stored locally on your device — it\'s never synced to the cloud.',
  aiCoachSaveHint: 'Save to apply changes.',
  ...
  coachWeeklyMetricDays: 'Days',
  coachWeeklyMetricDuration: 'Avg time',
  coachWeeklyMetricPrs: 'PRs',
  coachWeeklyMetricVolume: 'Volume',
  ...
  coachHistoryTitle: 'Coach history',
  coachHistoryHint: 'Recent AI and local recommendations',
  coachHistoryEmpty: 'No saved recommendations. The coach appears automatically after workouts and weekly.',
  coachHistoryFilterAll: 'All',
  coachHistoryFilterPostWorkout: 'Post-workout',
  coachHistoryFilterWeekly: 'Weekly',
  ...
  profileAboutLicenses: 'Exercise media licenses',
  licenseExerciseVideos: 'Demonstration videos:',
  licenseIllustrations: 'Illustrations:',
  licenseMit: '(license',
  licenseAnd: 'and',
  ...
  profileSettingsGroupAi: 'AI Coach',
  profileSettingsGroupData: 'Data & backup',
  profileSettingsGroupAppearance: 'Appearance & language',
  profileSettingsHintAppearance: 'Theme, contrast, language',
  profileSettingsHintTraining: 'Units, sound, screen',
  profileSettingsHintReminders: 'Notifications, reminders',
  profileSettingsHintAi: 'API key, model, provider',
  profileSettingsHintData: 'Import, export, deletion',
  ...
  exerciseStarterDeclineBenchPress: 'Decline barbell bench press',
  exerciseStarterPecDeck: 'Pec deck fly',
  // Back
  ...
  exerciseStarterDumbbellRow: 'Bent-over dumbbell row',
  exerciseStarterTbarRow: 'T-bar row',
  exerciseStarterStraightArmPulldown: 'Cable straight-arm pulldown',
  exerciseStarterShrug: 'Barbell shrug',
  // Shoulders
  ...
  exerciseStarterUprightRow: 'Upright row',
  // Arms
  ...
  exerciseStarterConcentrationCurl: 'Concentration curl',
  exerciseStarterPreacherCurl: 'Preacher curl',
  exerciseStarterOverheadTricepExtension: 'Overhead triceps extension',
  exerciseStarterTricepKickback: 'Triceps kickback',
  // Legs
  ...
  exerciseStarterFrontSquat: 'Front squat',
  exerciseStarterStepUp: 'Step-up',
  // Core
  ...
  exerciseStarterReverseCrunch: 'Reverse crunch',
  exerciseStarterLyingLegRaise: 'Lying leg raise',
  // Full body
  ...
  // Cardio
  exerciseStarterStairClimbing: 'Stair climbing',
  exerciseStarterRunning: 'Running',
  exerciseStarterCycling: 'Cycling',
  exerciseStarterRowingMachine: 'Rowing machine',
  exerciseStarterElliptical: 'Elliptical',
  exerciseStarterJumpRope: 'Jump rope',
  exerciseStarterJumpingJacks: 'Jumping jacks',
  exerciseStarterHighKnees: 'High knees',
  ...
  exerciseAddedToDay: (name: string) => `Added: ${name}`,
  exerciseReplacedInDay: (name: string) => `Replaced with: ${name}`,
  ...
  planShowOnTraining: 'Show on Training',
  planHideFromTraining: 'Hide from Training',
  ...
  customWorkoutSetsSectionCount: (done: number, total: number) => `Sets · ${done}/${total}`,
  customWorkoutProgressAria: (done: number, total: number) => `Progress: ${done} of ${total} sets`,
  ...
  customDurationUnitSec: 'sec',
  customDurationUnitLabel: 'Duration unit',
  customDurationUnitHint: 'Choose minutes for cardio exercises (e.g. running, cycling)',
  durationSecValue: (sec: number) => `${sec}s`,
  durationMinValue: (mins: number) => `${mins} min`,
  customSetLabelDurationMin: (mins: number, name: string) => `Hold ${mins} min · ${name}`,
  customMinDurationHintMin: (mins: number) => `Minimum ${mins} min — longer is OK too`,
  customWorkoutLessMin: 'One minute less',
  customWorkoutMoreMin: 'One minute more',
  customWorkoutStartTimerMin: 'Start',
  customWorkoutStopTimerMin: 'Stop',
  customWorkoutDurationMin: 'Duration (min)',
  exerciseMetricDurationMin: 'Duration (min)',
  ...
  followLoadError: 'Failed to load',
  followLoadErrorHint: 'Check your internet connection and try again.',
  followRetry: 'Try again',
...
Write in English. Be specific and encouraging.`,
  ...
  exerciseDemoTechnique: 'Technique',
  exerciseDemoPlay: 'Play animation',
  exerciseDemoPause: 'Pause animation',
  exerciseDemoExpand: 'Show animation',
  exerciseDemoCollapse: 'Hide animation',
  exerciseDemoBadge: 'Demo',
  exerciseDemoAttribution: 'Video: free-exercise-db-with-videos (MIT). Illustrations: @bryllim/workout-guide, RepDB (CC BY-SA 4.0 / RepDB)',
  exerciseHasDemo: 'This exercise has a demonstration animation',
  exerciseNoDemo: 'User exercise — no animation',
  exerciseTechPushup1: 'Body in a straight line from head to heels — don\'t drop hips',
  exerciseTechPushup2: 'Elbows at ~45° angle, don\'t flare them outward',
  exerciseTechPushup3: 'Lower to chest, push to full extension',
  exerciseTechPullup1: 'Full range — chin over bar, full hang at bottom',
  exerciseTechPullup2: 'Elbows drive down and back, not outward',
  exerciseTechPullup3: 'Control the descent — don\'t drop freely',
  exerciseTechSquat1: 'Knees over toes, not caving inward',
  exerciseTechSquat2: 'Chest up, back straight, slight forward lean',
  exerciseTechSquat3: 'Hips back as if sitting into a chair',
  exerciseTechPlank1: 'Tight core and glutes — body like a plank',
  exerciseTechPlank2: 'Don\'t drop hips or raise glutes',
  exerciseTechPlank3: 'Look at the floor, neck aligned with spine',
  exerciseTechLunge1: 'Step forward, front knee over heel',
  exerciseTechLunge2: 'Back leg straight, heel lifted',
  exerciseTechLunge3: 'Torso upright, don\'t lean forward',
  exerciseTechDip1: 'Elbows close to torso, don\'t flare outward',
  exerciseTechDip2: 'Lower to 90° elbow bend — no deeper',
  exerciseTechDip3: 'Shoulders down, not rising toward ears',
  exerciseTechBurpee1: 'Smooth transition: push-up → jump → up',
  exerciseTechBurpee2: 'Explode upward in the jump with arms overhead',
  exerciseTechCurl1: 'Elbows pinned to sides — don\'t swing arms',
  exerciseTechCurl2: 'Control the lowering — negative phase is key',
  exerciseTechPress1: 'Press overhead, not in front of you',
  exerciseTechPress2: 'Shoulders down, core tight — don\'t arch lower back',
  exerciseTechRow1: 'Drive elbows back, squeezing shoulder blades',
  exerciseTechRow2: 'Torso at a fixed angle — don\'t rock',
  exerciseTechRaise1: 'Raise to shoulder level — not higher',
  exerciseTechRaise2: 'Control the descent, don\'t drop the weight',
  exerciseTechCrunch1: 'Lift shoulders, not whole body — small range',
  exerciseTechCrunch2: 'Core engaged, neck relaxed — don\'t pull head',
  exerciseTechJumpingJacks1: 'Arms overhead, feet wide — full range',
  exerciseTechJumpingJacks2: 'Land softly on slightly bent knees',
  exerciseTechJumpRope1: 'Low jumps — just a few inches off the ground',
  exerciseTechJumpRope2: 'Legs together, land on the balls of your feet',
  exerciseTechRunning1: 'Push off the balls of your feet, not heels',
  exerciseTechRunning2: 'Arms close to body, drive forward not upward',
  exerciseTechCycling1: 'Smooth cadence — don\'t jerk the pedals',
  exerciseTechCycling2: 'Back straight, core engaged',
  exerciseTechGeneric1: 'Control the movement through full range',
  exerciseTechGeneric2: 'Maintain muscle tension throughout the rep',
}
