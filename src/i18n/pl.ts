...
export const plDict = {
  ...
  // Quick CTA w hero statusu
  homeQuickCtaStart: (program: string, day: number) => `Rozpocznij · ${program} · Dzień ${day}`,
  homeQuickCtaResume: (program: string, set: number, total: number) => `Dokończ · ${program} · Seria ${set}/${total}`,
  homeQuickCtaTest: (program: string) => `Rozpocznij test · ${program}`,
  homeQuickCtaSetup: (program: string) => `Skonfiguruj · ${program}`,
  homeQuickCtaScroll: 'Pokaż szczegóły',
  homeQuickCtaTrainAnyway: (program: string) => `Trenuj mimo to · ${program}`,
