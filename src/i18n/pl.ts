...
export const plDict = {
  ...
  homeActivitySectionAria: 'Twoja aktywność — seria, łańcuch tygodni i metryki',
  homeCommunityTitle: 'Społeczność',
  homeCommunitySectionAria: 'Społeczność — wyzwanie tygodnia i plany społeczności',
  homeMoreTraining: 'Więcej treningów',
  homeRowNewPlan: 'Nowy plan',
  homeRowNewPlanHint: 'Zbuduj własny trening',
  homeRowProgramsHint: 'Włącz kolejny program',
  homeRowLibraryHint: 'Przeglądaj ćwiczenia',
  ...
  // Kontekstowe nagłówki statusu dnia — headline + subtitle
  homeGreetingMorning: (name: string) => `Dzień dobry, ${name}`,
  homeGreetingAfternoon: (name: string) => `Dzień dobry, ${name}`,
  homeGreetingEvening: (name: string) => `Dobry wieczór, ${name}`,
  homeGreetingGeneric: (name: string) => `Cześć, ${name}`,
  homeStatusReadyHeadline: (day: number, total: number) => `Dzień ${day}/${total}`,
  homeStatusReadySubtitle: (program: string) => `${program} — gotowy do treningu`,
  homeStatusResumeHeadline: (program: string) => `Dokończ ${program}`,
  homeStatusResumeSubtitle: (set: number, total: number) => `Seria ${set}/${total}`,
  homeStatusResumeStaleSubtitle: 'Niedokończony trening — sprawdź poniżej',
  homeStatusResumeAndReadySubtitle: (otherProgram: string) => `Albo zacznij ${otherProgram}`,
  homeStatusTestReadyHeadline: 'Test maksymalny',
  homeStatusTestReadySubtitle: (program: string) => `${program} — gotowy`,
  homeStatusTestRestHeadline: 'Test po odpoczynku',
  homeStatusTestRestSubtitle: (program: string, when: string) => `${program} — ${when}`,
  homeStatusAllPausedSubtitle: 'Wznów, gdy będziesz gotowy',
  homeStatusSetupHeadline: 'Skonfiguruj program',
  homeStatusSetupSubtitle: 'Wybierz poziom, aby zacząć',
  homeStatusSetupMixedHeadline: 'Wznów lub skonfiguruj',

  // Adaptacyjny cel 14-dniowy
  homeGoalNin14: (n: number) =>
    n === 1
      ? '1 trening w 14 dni'
      : n >= 2 && n <= 4
        ? `${n} treningi w 14 dni`
        : `${n} treningów w 14 dni`,
  ...
  homeTipTitleWelcome: 'Witaj w SmartReps',
  homeTipWelcomeTraining: 'Karta programu poniżej. Zrób test max, wybierz cykl i rozpocznij.',
  homeTipWelcomeProgress: 'Historia treningów, osiągnięcia, analiza AI i trendy RPE.',
  homeTipWelcomePlans: 'Własne plany, gotowe programy i społeczność.',
  homeTipWelcomeProfile: 'Konto, synchronizacja, AI Coach i ustawienia.',
  homeTipWelcomeHint: 'Nawigacja między sekcjami na dole ekranu.',
  homeTipWelcomeCta: 'Rozpocznij pierwszy trening',
  ...
  trainAnywayNew: 'Trenuj mimo to',
  abandonResumeTrainAnywayTitle: 'Porzucić niedokończoną sesję?',
  abandonResumeTrainAnywayBody:
    'Zaczniesz Dzień od nowa mimo przerwy. Przerwa odliczy się od dziś po tym treningu.',
  ...
  forceRestRestartHint: 'Przerwa odliczy się od dziś po tym treningu.',
  ...
  resumePromptBodyCustom: (planName: string, day: number, set: number, total: number) =>
    `${planName} · Dzień ${day} · seria ${set}/${total}. Wznowić?`,
  resumePromptBodyFree: (exercises: number, sets: number) =>
    `Trening swobodny · ${exercises} ćw. · ${sets} serii. Wznowić?`,
  ...
  warmupGeneratorTitle: 'Generator rozgrzewki',
  warmupGeneratorHint: 'Wpisz ciężar roboczy — wygenerujemy drabinkę rozgrzewkową.',
  warmupGeneratorWorkingWeight: 'Ciężar roboczy',
  warmupGeneratorBarWeight: 'Waga sztangi',
  warmupGeneratorPlateStep: 'Skok talerza',
  warmupGeneratorGenerate: 'Generuj',
  warmupGeneratorAddAll: 'Dodaj wszystkie serie',
  warmupGeneratorAddSet: 'Użyj tej serii',
  warmupGeneratorAdded: 'Dodano serie rozgrzewkowe',
  warmupGeneratorEmptyBar: 'Pusta sztanga',
  warmupGeneratorPercent: (pct: number) => `${pct}% roboczego`,
  // RPE / RIR
  rpeLabel: 'RPE',
  ...
  rpeRirModeRir: 'RIR',
  rpeRirClear: 'Wyczyść',
  rpeDescription: (v: number): string => {
    if (v <= 3) return 'Bardzo lekko'
    if (v <= 5) return 'Lekko'
    if (v <= 7) return 'Umiarkowanie'
    if (v === 8) return '2 powt. w rezerwie'
    if (v === 9) return '1 powt. w rezerwie'
    return 'Maksymalny wysiłek'
  },
  rirDescription: (v: number): string => {
    if (v === 0) return 'Nic nie zostało'
    if (v === 1) return '1 powt. w rezerwie'
    if (v === 2) return '2 powt. w rezerwie'
    if (v <= 5) return `${v} powt. w rezerwie`
    return 'Bardzo lekko'
  },
  setNoteLabel: 'Notatka do serii',
  ...
  dropsetWeightReduction: 'Redukcja ciężaru',
  // Estimated 1RM
  est1rmTitle: 'Szacowany 1RM',
  est1rmHint: 'Na podstawie ostatnich serii (wzór Epley/Brzycki)',
  est1rmExercise: 'Ćwiczenie',
  est1rmValue: 'Szac. 1RM',
  est1rmReps: 'Powt.',
  est1rmWeight: 'Ciężar',
  est1rmDate: 'Data',
  ...
  forecastRepsValue: (n: string) => `${n} powt.`,
  forecastDurationValue: (sec: string) => `${sec} s`,
  ...
  workoutHint:
    'Ustaw wynik i naciśnij Zrobione. Serie „równo N” wymagają dokładnie N.',
  ...
  testSquats: 'Test przysiadów',
  testPrompt: 'Ile powtórzeń zrobisz za jednym razem na pełnej amplitudzie?',
  testHowTo: 'Wykonaj maksymalną liczbę powtórzeń bez przerwy. Użyj poprawnej techniki — niepełne powtórzenia nie liczą się.',
  ...
  warmupProgress: (done: number, total: number) => `${done}/${total} ukończone`,
  ...
  loginOtpAriaDigitWithValue: (n: number, value: string) =>
    `Cyfra ${n}${value ? `: ${value}` : ''}`,
  ...
  onboardingLanguagePl: 'PL',
  onboardingLanguageEn: 'EN',
  ...
  onboardingSquatsDesc: 'Od testu maksymalnego do 300 przysiadów — 15 cykli',
  onboardingReadyLine: 'Twój plan czeka. Zaczynamy?',
  onboardingCloudRestoreFailed: 'Nie udało się przywrócić danych z chmury',
  onboardingCloudRestoreFailedHint: 'Spróbuj ponownie w Profilu → Synchronizacja',
  onboardingCloudRestoreRetry: 'Ponów',
  ...
  onboardingPickProgramHint: 'Wybierz co najmniej jeden program, aby przejść dalej',
  ...
  onboardingNextBulletAiProgress: 'Trener AI analizuje treningi; wykresy, rekordy i odznaki śledzą progres',
  onboardingSlide1Title: 'Gotowe cykle treningowe',
  onboardingSlide1Desc: 'Od testu max do 100 pompek — plan każdego dnia gotowy.',
  onboardingSlide2Title: 'Trener AI',
  onboardingSlide2Desc: 'Analiza treningów, raporty tygodniowe i sugestie.',
  onboardingSlide3Title: 'Postępy i odznaki',
  onboardingSlide3Desc: 'Wykresy, rekordy i odznaki śledzą każdy krok.',
  onboardingAiPreviewWeek: 'Tydzień 4',
  onboardingAiPreviewSuggestion: 'Sugestia: dodaj 1 sesję podciągania dla równowagi',
  onboardingAiPreviewCoachName: 'Trener SmartReps',
  onboardingAiPreviewSummary: 'Świetny tydzień — objętość i progres w normie. Tak trzymaj!',
  onboardingAiPreviewStrength: 'Wydolność pompki rośnie 3 tyg. z rzędu',
  onboardingAiPreviewStrengthLabel: 'Mocne strony',
  onboardingAiPreviewSuggestionLabel: 'Sugestia trenera',
  onboardingAiPreviewStatSessions: 'Sesje',
  onboardingAiPreviewStatVolume: 'Objętość',
  onboardingAiPreviewStatPr: 'Rekord',
  onboardingBadgeFirst100: 'Pierwsze 100',
  onboardingBadgeStreak: 'Seria tygodni',
  onboardingBadgePr: 'Rekord PR',
  onboardingBadgeCycle: 'Cykl ukończony',
  onboardingChartLabel: '8 tyg.',
  onboardingChartReps: 'powt.',
  onboardingCarouselAria: 'Przewiń slajdy — funkcje aplikacji',
  onboardingSlideAria: (current: number, total: number) => `Slajd ${current} z ${total}`,
  onboardingSwipeHint: 'Przesuń, aby zobaczyć więcej',
  ...
  errorActivateLevel: 'Nie udało się zmienić poziomu. Spróbuj ponownie.',
  notFoundTitle: 'Nie znaleziono strony',
  notFoundBody: 'Ten adres nie istnieje w SmartReps. Wróć do ekranu Trening.',
  chartTestOverTime: 'Postęp ostatniej serii',
  progressLastSetChartHint: 'Powtórzenia z ostatniej serii w każdej sesji',
  progressLastSetChartAria: (count: number) => `Wykres postępu ostatniej serii, ${count} sesji`,
  progressLastSetChartTooltip: 'Ostatnia seria',
  progressProgramSectionTitle: (name: string) => name,
  progressScopeLabel: 'Zakres',
  progressScopeActivity: 'Aktywność',
  progressScopePushups: 'Pompki',
  progressScopePullups: 'Podciąganie',
  progressScopeSquats: 'Przysiady',
  ...
  techniqueSquatsTitle: 'Technika przysiadu',
  techniqueSquatsStep1:
    'Stopy na szerokość barków, plecy proste, wzrok przed siebie.',
  techniqueSquatsStep2:
    'Opuszczaj biodra w dół i w tył, jakbyś siadał na krześle. Kolana nie schodzą przed palce stóp.',
  techniqueSquatsStep3:
    'Wstań dynamicznie, prostując biodra i kolana — pełna amplituda bez zatrzymywania na dole.',
  techniqueSquatsPoseStart: 'Start',
  techniqueSquatsPoseBottom: 'Dół',
  techniqueSquatsPoseTop: 'Góra',
  helpTechniqueSquats: 'Technika przysiadu',
  ...
  seoTechniqueSquatsTitle: 'Technika przysiadu',
  seoTechniqueSquatsDescription:
    'Prawidłowa technika przysiadu — pozycja stóp, kolana, tempo i pełna amplituda. Poradnik dla początkujących.',
  ...
  workoutDayLabel: 'Dzień',
  workoutSetOfTotal: (current: number, total: number) => `Seria ${current} z ${total}`,
  ...
  collapseTimer: 'Zwiń',
  restReady: 'Gotowe!',
  restStartSet: 'Rozpocznij serię',
  restSetLabel: (current: number, total: number) => `Seria ${current} z ${total}`,
  restLastSeconds: 'Koniec przerwy za chwilę',
  ...
  streakAtRiskDaysLeft: (days: number) =>
    days === 1
      ? 'Został 1 dzień!'
      : days <= 4
        ? `Zostały ${days} dni`
        : `Zostało ${days} dni`,
  streakAtRiskLoss: (weeks: number) =>
    `Bez treningu stracisz serię ${weeks} tyg.`,
  streakAtRiskFreezeNote: 'Ochrona serii może ją uratować automatycznie.',
  streakChainAriaRisk: (current: number, best: number, days: number) =>
    `Seria treningowa zagrożona: ${current} tygodni z rzędu, rekord: ${best} tygodni, zostało ${days} dni`,
  streakDaysShort: (days: number) => `${days}d`,
  streakHeroSaveRibbon: (weeks: number) =>
    `Ten trening ratuje serię ${weeks} tyg.`,
  ...
  // Streak freeze (Pro)
  streakFreezeTitle: 'Ochrona serii',
  streakFreezeAvailable: (count: number) =>
    count === 1 ? '1 zamrożenie w zapasie' : `${count} zamrożenia w zapasie`,
  streakFreezeAvailableNone: 'Brak zamrożeń — nowe co miesiąc',
  streakFreezeHowItWorks:
    'Pominiesz tydzień? Zamrożenie automatycznie uratuje serię. Zbierasz 1 co miesiąc, maks. 3 w zapasie.',
  streakFreezeCellTitle: (weekStart: string) => `Tydzień ${weekStart}: zamrożony`,
  streakFreezeSavedToast: 'Zamrożenie serii uratowało Twój streak!',
  streakFreezeLegend: 'Zamrożony',
  streakFreezeFreeHint: 'Z SmartReps Pro seria przetrwa pominięty tydzień — automatycznie.',
  // Profile titles — unlocked by achievements, selectable in Pro
  title_first_session: 'Adept',
  title_habit_3_in_14: 'Regularny',
  title_first_custom_session: 'Własną drogą',
  title_first_squat: 'Adept przysiadu',
  title_first_publish: 'Twórca',
  title_first_like: 'Doceniony',
  title_first_follower: 'Zauważony',
  title_social_butterfly: 'Towarzyski',
  title_first_review: 'Krytyk',
  title_weekend_warrior: 'Weekendowy wojownik',
  title_streak_4: 'W rytmie',
  title_streak_12: 'Zawzięty',
  title_cycle_closed_strong: 'Finisher',
  title_goal_pullups_30: 'Łowca drążka',
  title_pr_repeat_3: 'Łamacz rekordów',
  title_workshop_custom: 'Konstruktor',
  title_plan_with_legs: 'Nogi ze stali',
  title_both_programs: 'Dwubojowiec',
  title_challenge_first: 'Pretendent',
  title_habit_builder: 'Budowniczy nawyków',
  title_trainer_25: 'Trener',
  title_speed_demon: 'Demon szybkości',
  title_streak_26: 'Nieugaszony',
  title_streak_52: 'Żelazna wola',
  title_sessions_100: 'Setnik',
  title_goal_pushups_100: 'Legenda pompek',
  title_goal_pullups_50: 'Mistrz drążka',
  title_goal_squats_300: 'Król przysiadu',
  title_volume_10k: 'Tytan objętości',
  title_cycles_5: 'Pielgrzym cykli',
  title_poly_publisher: 'Mistrz katalogu',
  title_challenge_winner: 'Czempion',
  title_triple_threat: 'Potrójne zagrożenie',
  title_pr_master: 'Mistrz rekordów',
  title_cycle_master: 'Mistrz cykli',
  title_followed_by_25: 'Lider',
  title_community_pillar: 'Filar społeczności',
  title_comeback_stronger: 'Powrót legendy',
  title_legend_full_circle: 'Pełne koło',
  title_legend_quiet_master: 'Cichy mistrz',
  title_legend_grandmaster: 'Arcymistrz',
  title_legend_community: 'Legenda społeczności',
  // Title picker UI
  profileTitleLabel: 'Tytuł profilu',
  profileTitleNone: 'Bez tytułu',
  profileTitlePickTitle: 'Wybierz tytuł',
  profileTitlePickHint: 'Tytuły odblokowujesz osiągnięciami — wybrany widać na Twoim profilu publicznym.',
  profileTitleUnlockedBy: (name: string) => `Za osiągnięcie: ${name}`,
  ...
  profileTitleSaved: 'Tytuł zapisany',
  profileTitleSaveError: 'Nie udało się zapisać tytułu. Spróbuj ponownie.',
  profileTitleProRequired: 'Tytuły profilu wymagają planu Pro',
  ...
  muscleBalanceWeeklySets: (sets: string) => `${sets} serii/tyg.`,
  muscleBalanceWarningNamed: (groups: string) => `Niedotrenowane: ${groups} — zadbaj o zrównoważony plan.`,
  ...
  importBackupFileAria: 'Wybierz plik backupu do importu',
  ...
  customPreviousDurationMin: (mins: number) => `${mins} min`,
  ...
  // ── Trening swobodny (ad-hoc / pusty trening) ──
  freeWorkoutTitle: 'Trening swobodny',
  freeWorkoutHomeHint: 'Zacznij od zera — dodawaj ćwiczenia w trakcie',
  freeWorkoutHeroBadge: 'Bez planu',
  freeWorkoutHeroHint: 'Pusty trening — czas leci, ćwiczenia dodajesz po drodze',
  freeWorkoutStart: 'Rozpocznij',
  freeWorkoutResume: 'Kontynuuj',
  freeWorkoutLoadError: 'Nie udało się wczytać treningu swobodnego. Spróbuj ponownie.',
  freeExerciseOptionsAria: 'Opcje ćwiczenia',
  freeSetsListAria: 'Zapisane serie',
  freeSetChipAria: (n: number, summary: string) => `Seria ${n}: ${summary} — dotknij, aby edytować`,
  freeRepsLabel: 'Powtórzenia',
  freeWeightLabel: (unit: string) => `Ciężar (${unit})`,
  freeDurationLabel: (unit: 'sec' | 'min'): string => (unit === 'min' ? 'Czas (min)' : 'Czas (sek)'),
  freeNextSetLabel: (n: number) => `Seria ${n}`,
  freeSetLogged: (n: number, exerciseName: string) => `${exerciseName} — seria ${n} zapisana`,
  freeUndoSet: 'Cofnij',
  freeLessValue: 'Zmniejsz wartość',
  freeMoreValue: 'Zwiększ wartość',
  freeLessWeight: 'Zmniejsz ciężar',
  freeMoreWeight: 'Zwiększ ciężar',
  freeHeaderStats: (exercises: number, sets: number) => `${exercises} ćw. · ${sets} serii`,
  freeMoveExerciseUp: 'Przenieś wyżej',
  freeMoveExerciseDown: 'Przenieś niżej',
  freeRemoveExercise: 'Usuń ćwiczenie',
  freeRemoveExerciseTitle: 'Usunąć ćwiczenie?',
  freeRemoveExerciseConfirm: 'Usunie też zapisane serie tego ćwiczenia.',
  freeEmptyTitle: 'Zaczynasz od zera',
  freeEmptyHint: 'Dodaj pierwsze ćwiczenie — czas treningu już leci.',
  freeRecentExercises: 'Ostatnio używane',
  freeFinishWorkout: 'Zakończ',
  freeRestNextLabel: 'Odpoczynek — kolejna seria lub ćwiczenie',
  freeFinishTitle: 'Zakończyć trening?',
  freeFinishConfirm: (exercises: number, sets: number) =>
    `${exercises} ćw., ${sets} serii — trening trafi do historii. Potem możesz zapisać go jako plan.`,
  freeFinishEmptyConfirm: 'Nie zapisano żadnej serii — trening zostanie porzucony.',
  freeLeaveTitle: 'Opuścić trening?',
  freeLeaveDesc: 'Trening pozostaje aktywny — wznowisz go ze strony głównej.',
  freeLeaveEmptyDesc: 'Nie zapisano żadnej serii — trening zostanie porzucony.',
  freeLeaveAction: 'Wyjdź',
  freeDiscardAction: 'Porzuć trening',
  freeEditSetTitle: (n: number) => `Seria ${n}`,
  freeSaveSet: 'Zapisz',
  freeSummaryTitle: 'Trening zaliczony',
  freeStatDuration: 'Czas',
  freeStatExercisesSets: 'Ćwiczenia · serie',
  freeStatVolume: 'Tonaż',
  freeStatReps: 'Powtórzenia',
  freeSummaryExercises: 'Wykonane ćwiczenia',
  freeSavePlanTitle: 'Zapisz jako plan',
  freeSavePlanDesc: 'Zachowaj ten układ ćwiczeń i serii jako plan do wielokrotnego użytku.',
  freeSavePlanSheetDesc: 'Nazwij plan — zapisane wartości staną się celami serii.',
  freeSaveAsPlan: 'Zapisz jako plan',
  freePlanNameLabel: 'Nazwa planu',
  freePlanNamePlaceholder: 'np. Siłownia — klatka i plecy',
  freePlanSaved: 'Plan zapisany — znajdziesz go w Planach → Moje.',
  freePlanSavedTitle: 'Plan zapisany',
  freePlanSavedHint: 'Możesz go teraz uruchamiać jak każdy inny plan treningowy.',
  freePlanSaveFailed: 'Nie udało się zapisać planu. Spróbuj ponownie.',
  freeOpenPlan: 'Otwórz Moje plany',
  ...
  pwaUpdateTitle: 'Nowa wersja aplikacji',
  pwaUpdateBody: 'Wczytaj najnowsze zmiany i ulepszenia.',
  pwaUpdateReload: 'Odśwież',
  pwaUpdateLater: 'Później',
  pwaUpdateApplying: 'Aktualizowanie…',
  pwaUpdateApplyingHint: 'Trwa wczytywanie nowej wersji. Strona odświeży się automatycznie.',
  ...
  bodyWeightDeleteConfirmTitle: 'Usunąć pomiar?',
  bodyWeightDeleteConfirm: 'Ten pomiar wagi zostanie trwale usunięty.',
  ...
  progressActivityTitle: 'Aktywność',
  ...
  progressRecordsEmptyHint: 'Ukończ pierwszy trening, aby zacząć śledzić postępy.',
  progressRecordsEmptyCta: 'Rozpocznij trening',
  ...
  progressTotalRepsAllTime: 'Wszystkie powtórzenia',
  progressVolumeTrend14d: 'Trend 14 dni',
  ...
  programTierBeginner: 'Początkujący',
  programTierIntermediate: 'Średniozaawansowany',
  programTierAdvanced: 'Zaawansowany',
  programTierBeginnerDesc: 'Budowanie podstaw — od pierwszych powtórzeń',
  programTierIntermediateDesc: 'Rozwój siły i objętości',
  programTierAdvancedDesc: 'Wytrzymałość i droga do celu',
  programTierRepsRange: (min: number, max: number | null, unit: string) =>
    max === null ? `${min}+ ${unit}` : `${min}–${max} ${unit}`,
  programTierCyclesCount: (n: number) => (n === 1 ? '1 cykl' : `${n} cykli`),
  programTierYourTier: 'Twój poziom',
  programTierLayout6day: '6-dniowy',
  programTierLayout3day: '3-dniowy',
  programTierLayout9day: '9-dniowy',
  programActivateThisLevel: 'Aktywuj ten poziom',
  programEnableAndTest: 'Włącz i zrób test',
  programDoMaxTest: 'Zrób test maksymalny',
  programRetest: 'Zrób test ponownie',
  programBackToCurrent: 'Wróć do obecnego',
  programChangeToThisLevel: 'Zmień na ten poziom',
  programRecommendedForYou: 'Zalecany dla Ciebie',
  programBackToWorkout: 'Wróć do treningu',
  planEstimatedWeeks: (min: number, max: number) => `~${min}–${max} tyg.`,
  ...
  resistanceBandsIntro:
    'Jeśli nie podciągasz się jeszcze samodzielnie, możesz zacząć od wariantu z gumą wspomagającą.',
  ...
  resistanceBandsNote:
    'Wariant z gumą traktuj jako trening techniczny uzupełniający właściwe cykle podciągania.',
  ...
  themeDark: 'Ciemny',
  themeLight: 'Jasny',
  themeHint: '„System" dopasowuje się do ustawień urządzenia.',
  highContrast: 'Wysoki kontrast',
  highContrastHint: 'Zwiększa czytelność tekstu i krawędzi.',
  trainingSettings: 'Ustawienia treningu',
  weightUnitLabel: 'Jednostka wagi',
  weightUnitKg: 'kg',
  weightUnitLb: 'lb',
  weightUnitHint: 'Wpływa na wyświetlanie ciężaru w treningu i planach.',
  languageLabel: 'Język',
  languagePl: 'Polski',
  languageEn: 'Angielski',
  languageHint: 'Zmienia język całej aplikacji natychmiast.',
  remindersSection: 'Przypomnienia',
  profileDangerZone: 'Niebezpieczne',
  profileDangerZoneHint: 'Te akcje są nieodwracalne. Używaj ostrożnie.',
  infoHintLabel: 'Pokaż podpowiedź',
  profileDataImportHint: 'Wczytaj kopię zapasową z pliku JSON.',
  profileDataExportJsonHint: 'Pełna kopia zapasowa (postęp, sesje, plany, ćwiczenia).',
  profileDataExportCsvHint: 'Eksportuj wszystkie sesje do pliku CSV.',
  profileDataClearHint: 'Usuwa wszystkie dane lokalne z tego urządzenia.',
  ...
  reminderHourHint: 'Dostajesz powiadomienie o tej godzinie, gdy trening jest dostępny.',
  ...
  celebrationSquats300: 'Cel 300 przysiadów osiągnięty!',
  ...
  starterEmptyHint: 'Zacznij od gotowego szablonu',
  starterEmptyHintDesc: 'Wybierz sprawdzony plan dopasowany do Twojego poziomu.',
  starterChipHint: 'Nie wiesz od czego zacząć?',
  starterChipCta: 'Wybierz szablon',
  starterSeeAll: 'Zobacz wszystkie szablony',
  starterCategoryAll: 'Wszystkie',
  starterCategoryHome: 'W domu',
  starterCategoryGym: 'Na siłowni',
  starterCategoryCardio: 'Cardio',
  starterEmptyCategory: 'Brak szablonów w tej kategorii',
  starterDifficultyBeginner: 'Początkujący',
  starterDifficultyIntermediate: 'Średniozaawansowany',
  starterDifficultyAdvanced: 'Zaawansowany',
  starterDaysPerWeek: (n: number) => (n === 1 ? '1 dzień/tyg' : `${n} dni/tyg`),
  starterEstimatedWeeks: (min: number, max: number) => `Cykl ~${min}–${max} tyg`,
  starterEquipmentBodyweight: 'Bez sprzętu',
  starterEquipmentDumbbells: 'Hantle',
  starterEquipmentBarbell: 'Sztanga',
  starterEquipmentFullGym: 'Pełna siłownia',
  starterEquipmentKettlebell: 'Kettlebell',
  starterPreview: 'Podgląd',
  starterActivate: 'Aktywuj',
  starterActivating: 'Aktywowanie…',
  starterActivatedToast: (name: string) => `Aktywowano plan „${name}”. Możesz zacząć trening.`,
  starterBadge: 'Z szablonu',
  starterPreviewTitle: 'Podgląd szablonu',
  starterPreviewDayLabel: (n: number) => `Dzień ${n}`,
  starterPreviewProgression: (delta: number) => `Progresja: +${delta} powt. po cyklu`,
  starterPreviewNoProgression: 'Bez auto-progresji',
  starterPreviewExercisesCount: (n: number) => (n === 1 ? '1 ćwiczenie' : n < 5 ? `${n} ćwiczenia` : `${n} ćwiczeń`),
  starterPreviewSetsCount: (n: number) => (n === 1 ? '1 seria' : n < 5 ? `${n} serie` : `${n} serii`),
  starterPreviewRestLabel: 'Przerwa',
  starterPreviewStartHint: 'Po aktywacji plan pojawi się w „Moje plany” i na ekranie Trening.',
  starterTemplateName_foundationBodyweight3d: 'Fundament Siłowy',
  starterTemplateDesc_foundationBodyweight3d: 'Pełne ciało bez sprzętu. Pompy, przysiady, deska — fundament siły i poprawnych wzorców ruchu. 3 razy w tygodniu.',
  starterTemplateName_calisthenicsStrength4d: 'Siła Kalisteniczna',
  starterTemplateDesc_calisthenicsStrength4d: 'Split push/pull/legs/core dla średniozaawansowanych. Hipertrofia i siła kalisteniczna, 4 dni w tygodniu.',
  starterTemplateName_pullupFoundations3d: 'Droga do Podciągnięcia',
  starterTemplateDesc_pullupFoundations3d: 'Droga do pierwszego podciągnięcia. Negatywy, warianty asystowane i ćwiczenia pomocnicze — 3 razy w tygodniu z progresją.',
  starterTemplateName_fullbodyDumbbell3d: 'Całe Ciało z Hantlami',
  starterTemplateDesc_fullbodyDumbbell3d: 'Siła całego ciała z hantlami. Nauka wzorców z obciążeniem, 3 pełne sesje w tygodniu.',
  starterTemplateName_upperLowerBarbell4d: 'Góra/Dół ze Sztangą',
  starterTemplateDesc_upperLowerBarbell4d: 'Klasyczny split góra/dół z sztangą. Siła i hipertrofia, 4 dni w tygodniu z progresją obciążenia.',
  starterTemplateName_pushPullLegs6d: 'PPL Zaawansowany',
  starterTemplateDesc_pushPullLegs6d: 'Zaawansowany PPL 5 dni/tyg. Maksymalna objętość dla hipertrofii — 16–24 serie na grupę mięśniową tygodniowo.',
  starterTemplateName_hiitExpress3d: 'HIIT Express',
  starterTemplateDesc_hiitExpress3d: 'Krótkie, intensywne sesje cardio. Tabata, interwały i obwody — 3 razy w tygodniu, 15–20 min.',
  starterTemplateName_coreConditioning3d: 'Core i Kondycja',
  starterTemplateDesc_coreConditioning3d: 'Mocny core (antyrotacja, antyekstenzja) + kondycja. Oparte na metodzie McGilla, 3 razy w tygodniu.',
  starterTemplateName_kettlebellPower3d: 'Moc Kettlebell',
  starterTemplateDesc_kettlebellPower3d: 'Eksplozywna moc z kettlebell. Wymachy, wyciskania i przysiady — 3 dni w tygodniu.',
  ...
  aiAnalysisEmptyCta: 'Rozpocznij trening',
  ...
  aiCoachSubsectionProvider: 'Dostawca',
  aiCoachSubsectionConnection: 'Połączenie',
  aiCoachSubsectionOptions: 'Opcje',
  aiCoachConnectionHint: 'Klucz API jest przechowywany lokalnie na urządzeniu — nie jest synchronizowany do chmury.',
  aiCoachSaveHint: 'Zapisz, aby zastosować zmiany.',
  ...
  coachPostWorkoutLocalStreak: (count: number) => `${pluralPl(count, 'sesja', 'sesje', 'sesji')} z rzędu z progresem — systematyczność działa. Tak trzymaj!`,
  ...
  coachPlateauBody: (programLabel: string, sessionCount: number, lastValue: number, bestValue: number, sessionsSinceBest: number) =>
    `${programLabel}: ${pluralPl(sessionCount, 'sesja', 'sesje', 'sesji')} bez progresu (ostatnio ${lastValue}, rekord ${bestValue}${sessionsSinceBest > 0 ? ` — ${pluralPl(sessionsSinceBest, 'sesję', 'sesje', 'sesji')} temu` : ''}). Rozważ deload (−40% objętości) albo zmianę planu.`,
  coachPlateauBodyRegression: (programLabel: string, sessionCount: number, lastValue: number, bestValue: number) =>
    `${programLabel}: ${pluralPl(sessionCount, 'sesja', 'sesje', 'sesji')}, wynik spadł do ${lastValue} (rekord ${bestValue}). Regres zwykle oznacza niedoregenerowanie — zaplanuj deload (−40% objętości) lub dodatkowy dzień przerwy.`,
  ...
  coachWeeklyReportSessions: (count: number, reps: number) => `${pluralPl(count, 'sesja', 'sesje', 'sesji')}, ${reps} powt. łącznie.`,
  ...
  coachWeeklyReportProgramEntry: (program: string, sessions: number, reps: number) => `${program}: ${pluralPl(sessions, 'sesja', 'sesje', 'sesji')}, ${reps} powt.`,
  ...
  coachWeeklyMetricDays: 'Dni',
  coachWeeklyMetricDuration: 'Śr. czas',
  coachWeeklyMetricPrs: 'Rekordy',
  coachWeeklyMetricVolume: 'Tonaż',
  coachWeeklyMetricDurationValue: (min: number) => `${min} min`,
  ...
  coachHistoryTitle: 'Historia trenera',
  coachHistoryHint: 'Ostatnie rekomendacje AI i lokalne',
  coachHistoryEmpty: 'Brak zapisanych rekomendacji. Trener pojawi się automatycznie po treningu i co tydzień.',
  coachHistoryFilterAll: 'Wszystkie',
  coachHistoryFilterPostWorkout: 'Po treningu',
  coachHistoryFilterWeekly: 'Tygodniowe',
  coachHistoryFilterPlateau: 'Plateau',
  ...
  coachHistoryShowOlder: (n: number) => `Pokaż starsze (${n})`,
  coachHistoryShowFewer: 'Zwiń listę',
  ...
  profileHeroEditProfile: 'Edytuj profil',
  profileHeroPublic: 'Profil publiczny',
  profileHeroPrivate: 'Profil prywatny',
  profileHeroFollowers: 'Obserwujący',
  profileHeroFollowing: 'Obserwowani',
  profileHeroFollowersAria: (n: number) => `Pokaż obserwujących (${n})`,
  profileHeroFollowingAria: (n: number) => `Pokaż obserwowanych (${n})`,
  profileHeroFollowHint: 'Ustaw profil jako publiczny, aby inni mogli Cię obserwować',
  ...
  profileAboutLicenses: 'Licencje materiałów ćwiczeniowych',
  licenseExerciseVideos: 'Wideo demonstracyjne:',
  licenseIllustrations: 'Ilustracje:',
  licenseMit: '(licencja',
  licenseAnd: 'i',
  ...
  profileSettingsGroupAi: 'Trener AI',
  profileSettingsGroupData: 'Dane i backup',
  profileSettingsGroupAppearance: 'Wygląd i język',
  profileSettingsHintAppearance: 'Motyw, kontrast, język',
  profileSettingsHintTraining: 'Jednostki, dźwięk, ekran',
  profileSettingsHintReminders: 'Powiadomienia, przypomnienia',
  profileSettingsHintAi: 'Klucz API, model, dostawca',
  profileSettingsHintData: 'Import, eksport, usuwanie',
  ...
  exerciseLibraryLoadError: 'Nie udało się wczytać ćwiczeń.',
  ...
  exerciseSortName: 'Nazwa A-Z',
  exerciseSortRecent: 'Ostatnio trenowane',
  exerciseSortSessions: 'Najwięcej treningów',
  ...
  exerciseStarterDeclineBenchPress: 'Wyciskanie sztangi na ławce skośnej ujemnej',
  exerciseStarterPecDeck: 'Rozpiętki na maszynie (Pec Deck)',
  // Plecy
  ...
  exerciseStarterDumbbellRow: 'Wiosłowanie hantlem w opadzie',
  exerciseStarterTbarRow: 'Wiosłowanie T-bar',
  exerciseStarterStraightArmPulldown: 'Prostowanie ramion na wyciągu górnym',
  exerciseStarterShrug: 'Wzruszanie barków ze sztangą',
  // Barki
  ...
  exerciseStarterUprightRow: 'Wiosłowanie wzdłuż ciała ze sztangą',
  // Ramiona
  ...
  exerciseStarterConcentrationCurl: 'Uginanie ramienia z hantlem z koncentracją',
  exerciseStarterPreacherCurl: 'Uginanie ramion na modlitewniku',
  exerciseStarterOverheadTricepExtension: 'Prostowanie ramion nad głową na wyciągu',
  exerciseStarterTricepKickback: 'Prostowanie ramienia w opadzie (kickback)',
  // Nogi
  ...
  exerciseStarterFrontSquat: 'Przysiad przedni ze sztangą',
  exerciseStarterStepUp: 'Wchodzenie na podwyższenie',
  // Core
  ...
  exerciseStarterReverseCrunch: 'Spięcia brzucha w leżeniu (reverse crunch)',
  exerciseStarterLyingLegRaise: 'Unoszenie nóg w leżeniu',
  // Całe ciało
  ...
  // Cardio
  exerciseStarterStairClimbing: 'Wchodzenie po schodach',
  exerciseStarterRunning: 'Bieganie',
  exerciseStarterCycling: 'Jazda na rowerze',
  exerciseStarterRowingMachine: 'Wioślarz (ergometr)',
  exerciseStarterElliptical: 'Orbitrek',
  exerciseStarterJumpRope: 'Skakanie na skakance',
  exerciseStarterJumpingJacks: 'Pajacyki',
  exerciseStarterHighKnees: 'Wysokie unoszenie kolan (high knees)',
  ...
  exerciseAddedToDay: (name: string) => `Dodano: ${name}`,
  exerciseReplacedInDay: (name: string) => `Zmieniono na: ${name}`,
  ...
  planShowOnTraining: 'Pokaż na Treningu',
  planHideFromTraining: 'Ukryj z Treningu',
  planMoreActions: 'Więcej',
  planStatusDraft: 'Szkic',
  planStatusActive: 'Aktywny',
  planStatusPaused: 'Wstrzymany',
  planBack: 'Wstecz',
  ...
  customWorkoutHint:
    'Ustaw wynik i naciśnij Zrobione. Poniżej celu = nieudana seria.',
  customWorkoutSetsSection: 'Serie',
  customWorkoutSetsSectionCount: (done: number, total: number) => `Serie · ${done}/${total}`,
  setLogDetailsTitle: 'Szczegóły serii',
  setLogDetailsRpeChip: (value: number) => `RPE ${value}`,
  setLogDetailsRirChip: (value: number) => `RIR ${value}`,
  setLogDetailsNoteChip: 'notatka',
  setRestAdjustSummary: (restSec: number, setsCount: number) =>
    `Przerwa: ${restSec}s · ${setsCount} serii`,
  customWorkoutProgressAria: (done: number, total: number) => `Postęp: ${done} z ${total} serii`,
  customWorkoutRestChip: (sec: number) => `Przerwa ${sec}s`,
  customWorkoutExerciseDone: (done: number, total: number) => `${done}/${total} serii`,
  customWorkoutExerciseCompleted: 'Ukończone',
  customWorkoutExerciseNow: 'Teraz',
  ...
  customDurationUnitMin: 'min',
  customDurationUnitSec: 'sek',
  customDurationUnitLabel: 'Jednostka czasu',
  customDurationUnitHint: 'Wybierz minuty dla ćwiczeń cardio (np. bieganie, rower)',
  durationSecValue: (sec: number) => `${sec}s`,
  durationMinValue: (mins: number) => `${mins} min`,
  customSetLabelDurationMin: (mins: number, name: string) => `Trzymaj ${mins} min · ${name}`,
  customMinDurationHintMin: (mins: number) => `Minimum ${mins} min — dłużej też OK`,
  customWorkoutLessMin: 'Minuta mniej',
  customWorkoutMoreMin: 'Minuta więcej',
  customWorkoutStartTimerMin: 'Start',
  customWorkoutStopTimerMin: 'Stop',
  customWorkoutDurationMin: 'Czas (min)',
  exerciseMetricDurationMin: 'Czas (min)',
  customWorkoutDurationDisplayMin: 'min',
  ...
  // ── Plan reviews ──
  communityReviewsTitle: 'Oceny',
  communityReviewsCount: (n: number) => (n === 1 ? '1 ocena' : `${n} ocen`),
  communityReviewsEmpty: 'Brak ocen — bądź pierwszy!',
  communityReviewAdd: 'Oceń plan',
  communityReviewEdit: 'Edytuj ocenę',
  communityReviewDelete: 'Usuń ocenę',
  communityReviewDeleteConfirm: 'Usunąć Twoją ocenę?',
  communityReviewDeleteConfirmMessage: 'Ta operacja jest nieodwracalna. Twoja ocena i komentarz zostaną trwale usunięte.',
  communityReviewRatingLabel: 'Ocena (1-5 gwiazdek)',
  communityReviewCommentLabel: 'Komentarz (opcjonalnie)',
  communityReviewCommentPlaceholder: 'Np. dobry plan na początek, ale za mało objętości…',
  communityReviewSubmit: 'Opublikuj ocenę',
  communityReviewUpdate: 'Zaktualizuj ocenę',
  communityReviewDone: 'Ocena opublikowana',
  communityReviewDeleted: 'Ocena usunięta',
  communityReviewSelfForbidden: 'Nie możesz ocenić własnego planu',
  communityReviewLoginRequired: 'Zaloguj się, aby ocenić',
  communityReviewCommentTooLong: 'Komentarz może mieć max 500 znaków',
  communityReviewYourReview: 'Twoja ocena',
  communityReviewAnonymous: 'Anonim',
  communityReviewStars: (n: number) => `${n} z 5 gwiazdek`,
  // ── Weekly challenge ──
  challengeTitle: 'Wyzwanie tygodnia',
  ...
  challengeEndsInShort: (days: number) =>
    days <= 0 ? 'Zakończone' : days === 1 ? 'za 1 dzień' : `za ${days} dni`,
  ...
  challengeLoginRequired: 'Zaloguj się, aby wziąć udział',
  challengeLeaderboard: 'Ranking',
  challengeLeaderboardEmpty: 'Brak wyników — bądź pierwszy!',
  challengeRankPosition: (n: number) => `Miejsce ${n}`,
  challengeAnonymous: 'Anonim',
  challengeYouLabel: 'Ty',
  challengeWeekKey: (week: string) => `Tydzień ${week}`,
  challengeProgressAria: (current: number, target: number) =>
    `Postęp: ${current} z ${target} powtórzeń, ${Math.round((current / target) * 100)} procent`,
  challengeLeaderboardGlobal: 'Globalny',
  challengeLeaderboardFollowing: 'Obserwowani',
  challengeLeaderboardFollowingEmpty: 'Nikt z obserwowanych nie wziął jeszcze udziału',
  challengeEnded: 'Wyzwanie zakończone',
  challengeEndedHint: 'Wyniki są zamknięte. Sprawdź ranking końcowy!',
  challengeAutoTracked: 'Śledzone automatycznie z Twoich treningów',
  challengeRecommended: 'Polecane',
  challengeYourAverage: (n: number) => `Twoja średnia: ${n}/tydz`,
  challengeYourBest: (n: number) => `Rekord do pobicia: ${n}`,
  challengeYourBestSet: (n: number) => `Twoja najlepsza seria: ${n}`,
  challengeYourBestDay: (n: number) => `Twój najlepszy dzień: ${n}`,
  challengeYourAvgSession: (n: number) => `Średnio na trening: ${n}`,
  challengeYourAvgSets: (n: number) => `Średnio: ${n} ${pluralPl(n, 'seria', 'serie', 'serii')}/tydz`,
  challengeYourLastWeek: (n: number) => `Ostatni tydzień: ${n}`,
  challengeDifficultyEasy: 'Łatwe',
  challengeDifficultyChallenging: 'Odpowiednie',
  challengeDifficultyHard: 'Trudne',
  challengeDifficultyUnknown: '—',
  challengeShowAll: (n: number) => `Pokaż wszystkie (${n})`,
  challengeShowTop: 'Pokaż polecane',
  challengeRecapTitle: 'Wyzwania tygodnia',
  challengeRecapAchieved: 'Wyzwanie ukończone!',
  challengeRecapDone: 'Ukończone',
  challengeRecapContributionReps: (n: number) => `+${n} powtórzeń z tego treningu`,
  challengeRecapContributionWorkoutTotal: (n: number) =>
    `Ten trening: ${pluralPl(n, 'powtórzenie', 'powtórzenia', 'powtórzeń')}`,
  challengeRecapContributionSession: '+1 trening',
  challengeRecapContributionDay: '+1 dzień',
  challengeRecapContributionSets: (n: number) =>
    `+${n} ${pluralPl(n, 'seria', 'serie', 'serii')} z tego treningu`,
  challengeRecapContributionMet: 'Warunek spełniony!',
  challengeRecapContributionClockHalf: (part: 'early' | 'late'): string =>
    part === 'early' ? 'Poranek zaliczony (1/2)' : 'Wieczór zaliczony (1/2)',
  challengeRecapContributionSurplus: (n: number) =>
    `+${n} ${pluralPl(n, 'powtórzenie', 'powtórzenia', 'powtórzeń')} ponad cel`,
  ...
  challengeRecapRemainingDays: (n: number) => `Jeszcze ${pluralPl(n, 'dzień', 'dni', 'dni')}!`,
  challengeRecapRemainingSets: (n: number) => `Jeszcze ${pluralPl(n, 'seria', 'serie', 'serii')}!`,
  ...
  challengeTypeVolume: 'Mistrz Objętości',
  challengeTypeConsistency: 'Żelazna Dyscyplina',
  challengeTypePrecision: 'Perfekcyjna Forma',
  challengeTypePersonalBest: 'Łowca Rekordów',
  challengeTypeMarathon: 'Maraton',
  challengeTypeMaxSet: 'Seria Mocy',
  challengeTypeGrinder: 'Młynarz',
  challengeTypeDaily: 'Codzienny Rytm',
  challengeTypeEarlyBird: 'Skowronek',
  challengeTypeNightOwl: 'Nocna Sowa',
  challengeTypeWeekend: 'Wojownik Weekendu',
  challengeTypeDouble: 'Dublet',
  challengeTypePerfectPair: 'Para Perfekcyjna',
  challengeTypeImprovement: 'Skok Progresu',
  challengeCatPower: 'Siła i objętość',
  challengeCatHabit: 'Rytm i nawyk',
  challengeCatSkill: 'Precyzja',
  challengeCatRecords: 'Rekordy',
  challengeDescVolume: 'Zrób jak najwięcej powtórzeń w tym tygodniu',
  challengeDescConsistency: (n: number) =>
    `Ukończ co najmniej ${pluralPl(n, 'trening', 'treningi', 'treningów')} w tym tygodniu`,
  challengeDescPrecision: 'Ukończ trening ze wszystkimi seriami zaliczonymi',
  challengeDescPersonalBest: 'Pobij swój rekord powtórzeń w pojedynczej serii',
  challengeDescMarathon: (n: number) => `Zrób co najmniej ${n} powtórzeń w jednym treningu`,
  challengeDescMaxSet: (n: number) => `Zrób co najmniej ${n} powtórzeń w jednej serii`,
  challengeDescGrinder: (n: number) => `Ukończ łącznie ${n} serii w tym tygodniu`,
  challengeDescDaily: (n: number) => `Trenuj w co najmniej ${n} różne dni tego tygodnia`,
  challengeDescEarlyBird: 'Ukończ trening rozpoczęty przed 9:00',
  challengeDescNightOwl: 'Ukończ trening rozpoczęty o 20:00 lub później',
  challengeDescWeekend: 'Ukończ trening w sobotę lub niedzielę',
  challengeDescDouble: 'Ukończ dwa treningi tego samego dnia',
  challengeDescPerfectPair: (n: number) =>
    `Ukończ ${pluralPl(n, 'trening', 'treningi', 'treningów')} ze wszystkimi seriami zaliczonymi`,
  challengeDescImprovement: (n: number) => `Zrób co najmniej ${n} powtórzeń więcej niż w zeszłym tygodniu`,
  challengeTypeSurplus: 'Premia za Więcej',
  challengeTypeDominator: 'Dominator',
  challengeTypeStrongFinish: 'Mocny Finał',
  challengeTypeSessionStarter: 'Mocny Start',
  challengeTypeBigDay: 'Wielki Dzień',
  challengeTypeWeekdayQuest: 'Misja Dnia',
  challengeTypeMorningMoves: 'Poranna Rozgrzewka',
  challengeTypeLunchBreak: 'Przerwa na Trening',
  challengeTypeEveningShift: 'Wieczorna Zmiana',
  challengeTypeAroundTheClock: 'Do Okoła Zegara',
  challengeTypeSundaySweat: 'Niedzielny Pot',
  challengeTypeHatTrick: 'Hat-Trick',
  challengeTypeFlawlessSets: 'Bez Skazy',
  challengeTypeSharpshooter: 'Snajper',
  challengeTypeBounceBack: 'Powrót po Upadku',
  challengeTypeMetronome: 'Metronom',
  challengeTypeVolumeRecord: 'Rekord Tygodnia',
  challengeTypeSessionRecord: 'Rekord Treningu',
  challengeTypeDayRecord: 'Rekord Dnia',
  challengeTypeBeatAverage: 'Ponad Średnią',
  challengeWeekdayPhrases: [
    'w poniedziałek',
    'we wtorek',
    'w środę',
    'w czwartek',
  ...
  challengeDescSurplus: (n: number) => `Zrób co najmniej ${n} powtórzeń ponad cele serii`,
  challengeDescDominator: 'Zrób serię na co najmniej 150% jej celu',
  challengeDescStrongFinish: 'Zakończ trening zaliczoną ostatnią serią',
  challengeDescSessionStarter: 'Rozpocznij trening zaliczoną pierwszą serią',
  challengeDescBigDay: (n: number) => `Zrób co najmniej ${n} powtórzeń w jednym dniu`,
  challengeDescWeekdayQuest: (day: string) => `Ukończ trening ${day} tego tygodnia`,
  challengeDescMorningMoves: 'Ukończ trening rozpoczęty przed 12:00',
  challengeDescLunchBreak: 'Ukończ trening rozpoczęty między 11:00 a 14:00',
  challengeDescEveningShift: 'Ukończ trening rozpoczęty między 18:00 a 22:00',
  challengeDescAroundTheClock: 'Ukończ trening przed 9:00 i trening po 20:00',
  challengeDescSundaySweat: 'Ukończ trening w niedzielę',
  challengeDescHatTrick: (n: number) =>
    `Ukończ ${pluralPl(n, 'trening', 'treningi', 'treningów')} ze wszystkimi seriami zaliczonymi`,
  challengeDescFlawlessSets: (n: number) =>
    `Zrób co najmniej ${pluralPl(n, 'serię', 'serie', 'serii')} na poziomie celu lub wyżej`,
  challengeDescSharpshooter: 'Zrób serię z dokładnie wymaganą liczbą powtórzeń',
  challengeDescBounceBack: 'Po niezaliczonej serii zalicz kolejną w tym samym treningu',
  challengeDescMetronome: 'Ukończ trening, w którym serie różnią się maks. o 2 powtórzenia',
  challengeDescVolumeRecord: 'Pobij swój rekord powtórzeń w tygodniu',
  challengeDescSessionRecord: 'Pobij swój rekord powtórzeń w jednym treningu',
  challengeDescDayRecord: 'Pobij swój rekord powtórzeń w jednym dniu',
  challengeDescBeatAverage: 'Przekrocz swoją średnią tygodniową liczbę powtórzeń',
  ...
  challengeProgressDays: (current: number, target: number) =>
    `${current} / ${target} ${pluralPl(target, 'dzień', 'dni', 'dni')}`,
  challengeProgressSets: (current: number, target: number) =>
    `${current} / ${target} ${pluralPl(target, 'seria', 'serie', 'serii')}`,
  challengeProgressImprovement: (current: number, target: number) =>
    `+${current} / +${target} powt.`,
  ...
  followDone: 'Obserwujesz',
  unfollowDone: 'Przestałeś obserwować',
  followErrorGeneric: 'Wystąpił błąd. Spróbuj ponownie.',
  followAnonymous: 'Anonim',
  followDisplayNameRequired: 'Ustaw nazwę wyświetlaną w ustawieniach, aby ustawić profil jako publiczny',
  followUnfollowFromList: 'Przestań obserwować',
  unfollowConfirm: 'Przestać obserwować tego użytkownika?',
  unfollowConfirmMessage: 'Nie będziesz już widzieć jego postępów w sekcji obserwowanych.',
  followFollowing: (n: number) => (n === 1 ? '1 obserwowany' : `${n} obserwowanych`),
  followFollowers: (n: number) => (n === 1 ? '1 obserwujący' : `${n} obserwujących`),
  followFollowersList: 'Obserwujący mnie',
  followFollowersEmpty: 'Nikt Cię jeszcze nie obserwuje',
  followFollowersEmptyHint: 'Opublikuj plan w katalogu społeczności lub ustaw profil jako publiczny, aby inni mogli Cię znaleźć.',
  followFollowersSheetTitle: (n: number) => (n === 0 ? 'Obserwujący' : n === 1 ? '1 obserwujący' : `${n} obserwujących`),
  followFollowingSheetTitle: (n: number) => (n === 0 ? 'Obserwowani' : n === 1 ? '1 obserwowany' : `${n} obserwowanych`),
  followFollowsYou: 'Obserwuje Cię',
  followStatsPullupMax: 'Max podciągania',
  followStatsTotalReps: 'Suma powtórzeń',
  ...
  followLoginRequired: 'Zaloguj się, aby obserwować',
  followCannotFollowSelf: 'Nie możesz obserwować samego siebie',
  followUserNotPublic: 'Ten użytkownik nie ma publicznego profilu',
  followPublicProfile: 'Profil publiczny',
  followPublicProfileHint: 'Pozwól innym widzieć Twoje postępy i Cię obserwować',
  followMakePublic: 'Ustaw profil jako publiczny',
  followMakePrivate: 'Ustaw profil jako prywatny',
  followDisplayName: 'Nazwa wyświetlana',
  followBio: 'Bio',
  followBioPlaceholder: 'Krótko o sobie — cel treningowy, doświadczenie…',
  followBioHint: 'Max 200 znaków',
  followSaveProfile: 'Zapisz profil',
  followProfileSaved: 'Profil zapisany',
  followStatsTotalSessions: 'Sesje',
  followStatsCurrentStreak: 'Seria tygodni',
  followStatsPushupMax: 'Max pompki',
  followFollowingList: 'Obserwowani',
  followLoadError: 'Nie udało się wczytać',
  followLoadErrorHint: 'Sprawdź połączenie z internetem i spróbuj ponownie.',
  followRetry: 'Spróbuj ponownie',
  ...
  communityPublishNeedPublicProfile: 'Aby opublikować plan, ustaw profil jako publiczny.',
  communityPublishNeedPublicProfileHint: 'Inni użytkownicy muszą wiedzieć, kto tworzy plany. Przejdź do Profil → Obserwuj i ustaw profil jako publiczny.',
  communityPublishGoToProfile: 'Przejdź do profilu',
  ...
  communityTeaserLoadError: 'Nie udało się wczytać propozycji.',
  ...
  achievement_goal_squats_300_title: 'Trzy setki przysiadów',
  achievement_goal_squats_300_desc: '300 przysiadów w teście max.',
  ...
  builtinExerciseSquats: 'Przysiady',
  ...
  termsBody4:
    'Plany treningowe w aplikacji zostały opracowane autorsko przez SmartReps na bazie ogólnodostępnej metodyki treningu kalistenicznego. SmartReps implementuje śledzenie postępu.',
  termsBody5:
...
Zwróć JSON w tym formacie (to jest przykład, podmień wartości):
{
  ...
  aiPromptPlanDurationLine: (min: number) => `- Czas sesji: ${min} min`,
...
Napisz po ${pl.aiPromptLanguageHint}. Bądź konkretny i zachęcający.`,
  ...
  exerciseDemoTechnique: 'Technika',
  exerciseDemoPlay: 'Odtwórz animację',
  exerciseDemoPause: 'Wstrzymaj animację',
  exerciseDemoExpand: 'Pokaż animację',
  exerciseDemoCollapse: 'Ukryj animację',
  exerciseDemoBadge: 'Demo',
  exerciseDemoAttribution: 'Wideo: free-exercise-db-with-videos (MIT). Ilustracje: @bryllim/workout-guide, RepDB (CC BY-SA 4.0 / RepDB)',
  exerciseHasDemo: 'To ćwiczenie ma animację demonstracyjną',
  exerciseNoDemo: 'Ćwiczenie użytkownika — bez animacji',
  exerciseTechPushup1: 'Ciało w prostej linii od głowy do pięt — nie opuszczaj bioder',
  exerciseTechPushup2: 'Łokcie pod kątem ~45°, nie rozstawiaj ich na boki',
  exerciseTechPushup3: 'Schodź do klatki, wypychaj do pełnego wyprostu',
  exerciseTechPullup1: 'Pełny zakres — broda nad drążek, pełny wyprost na dole',
  exerciseTechPullup2: 'Łokcie prowadzą w dół i do tyłu, nie w strony',
  exerciseTechPullup3: 'Kontroluj fazę opuszczania — nie opadaj swobodnie',
  exerciseTechSquat1: 'Kolana nad palcami stóp, nie schodzą do środka',
  exerciseTechSquat2: 'Klatka wypchnięta, plecy proste, tułów lekko pochylony',
  exerciseTechSquat3: 'Biodra cofają się jak do siadania na krześle',
  exerciseTechPlank1: 'Napięty brzuch i pośladki — ciało jak deska',
  exerciseTechPlank2: 'Nie opuszczaj bioder ani nie unoś pośladków',
  exerciseTechPlank3: 'Patrz w podłogę, szyja w linii z kręgosłupem',
  exerciseTechLunge1: 'Krok do przodu, kolano nad piętą',
  exerciseTechLunge2: 'Tylna noga prosta, pięta uniesiona',
  exerciseTechLunge3: 'Tułów pionowo, nie pochylaj się do przodu',
  exerciseTechDip1: 'Łokcie blisko tułowia, nie rozstawiaj na boki',
  exerciseTechDip2: 'Schodź do kąta 90° w łokciu — nie głębiej',
  exerciseTechDip3: 'Barki opuszczone, nie unoszą się do uszu',
  exerciseTechBurpee1: 'Płynne przejście: pompka → skok → w górę',
  exerciseTechBurpee2: 'W skoku wybij się maksymalnie w górę z rękami',
  exerciseTechCurl1: 'Łokcie przyklejone do tułowia — nie kołysz rękami',
  exerciseTechCurl2: 'Kontroluj opuszczanie — faza negatywna jest kluczowa',
  exerciseTechPress1: 'Wypychaj nad głowę, nie przed siebie',
  exerciseTechPress2: 'Barki opuszczone, rdzeń napięty — nie wyginaj lędźwi',
  exerciseTechRow1: 'Prowadź łokcie do tyłu, ściskając łopatki',
  exerciseTechRow2: 'Tułów pod stałym kątem — nie kołysz się',
  exerciseTechRaise1: 'Unieś do poziomu ramion — nie wyżej',
  exerciseTechRaise2: 'Kontroluj opuszczanie, nie rzucaj ciężaru',
  exerciseTechCrunch1: 'Unieś barki, nie całe ciało — mały zakres',
  exerciseTechCrunch2: 'Brzuch napięty, szyja luźna — nie ciągnij głowy',
  exerciseTechJumpingJacks1: 'Ręce nad głowę, stopy szeroko — pełny zakres',
  exerciseTechJumpingJacks2: 'Ląduj miękko na lekko ugiętych kolanach',
  exerciseTechJumpRope1: 'Niskie podskoki — wystarczy kilka centymetrów',
  exerciseTechJumpRope2: 'Nogi razem, ląduj na śródstopiu',
  exerciseTechRunning1: 'Wybij się ze śródstopia, nie z pięty',
  exerciseTechRunning2: 'Ręce blisko tułowia, ruch do przodu nie do góry',
  exerciseTechCycling1: 'Kadencja płynna — nie szarp pedałami',
  exerciseTechCycling2: 'Plecy proste, rdzeń napięty',
  exerciseTechGeneric1: 'Kontroluj ruch w pełnym zakresie',
  exerciseTechGeneric2: 'Utrzymuj napięcie mięśniowe przez całe powtórzenie',
  ...
  proFeatureUnlimitedPlans: 'Nielimitowane plany własne',
  ...
  proFeatureStreakFreeze: 'Ochrona serii (zamrożenie tygodnia)',
  proFeatureProfileTitle: 'Tytuły profilu z osiągnięć',
  proPlanMonthly: 'Miesięcznie',
  proPlanAnnual: 'Rocznie',
  proPlanLifetime: 'Dożywotnio',
  proPlanAnnualSavings: 'Oszczędzasz 67%',
  ...
  proPortalNoCustomer: 'Brak aktywnej subskrypcji do zarządzania.',
  ...
  proLimitStreakFreezeTitle: 'Ochrona serii w Pro',
  proLimitStreakFreezeDesc: 'Z Pro pominięty tydzień nie przerywa serii — zamrożenie używa się automatycznie. Zbierasz 1 co miesiąc, maks. 3.',
  proLimitProfileTitleTitle: 'Tytuły profilu w Pro',
  proLimitProfileTitleDesc: 'W Pro wybierzesz tytuł odblokowany osiągnięciami — widoczny na Twoim profilu publicznym.',
  proTeaserTitle: 'Funkcja Pro',
  proTeaserDescription: 'Ta funkcja wymaga planu Pro.',
  ...
  proRowStreakFreeze: 'Ochrona serii (streak freeze)',
  proRowProfileTitle: 'Tytuły profilu z osiągnięć',
  ...
  proPriceAnnualPromo: 'Cena startowa',
  proPriceLifetime: '299,99 zł',
  proPriceAnnualMonthly: '~6,67 zł/mies.',
  proLaunchOfferNote:
    'Oferta startowa — wkrótce roczny plan będzie kosztował 99,99 zł. Kupując teraz, zachowujesz niższą cenę na zawsze.',
  proPerYear: '/ rok',
  proPerMonth: '/ mies.',
