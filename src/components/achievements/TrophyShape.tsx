...
export function TrophyShape({
  ...
  return (
    <svg
      ...
      fill="none"
      className={cn('sr-trophy', `sr-trophy--${tier}`, className)}
    ...
    >
      ...
      {tier === 'bronze' && <MedalShape gradId={gradId} shineId={shineId} />}
      {tier === 'silver' && <ShieldShape gradId={gradId} shineId={shineId} />}
      {tier === 'gold' && <CupShape gradId={gradId} shineId={shineId} />}
      {tier === 'diamond' && <DiamondShape gradId={gradId} shineId={shineId} />}
    </svg>
  )
}
...
type ShapeProps = { gradId: string; shineId: string }

function MedalShape({ gradId, shineId }: ShapeProps) {
  return (
    <g>
      {/* Ribbon */}
      <path
      ...
      />
      {/* Medal body */}
      <circle cx="24" cy="28" r="13" fill={`url(#${gradId})`} stroke={GRADIENT_STOPS.bronze.lo} strokeWidth="0.8" />
      {/* Inner ring */}
...
function ShieldShape({ gradId, shineId }: ShapeProps) {
  return (
    <g>
      {/* Shield body */}
      <path
...
function CupShape({ gradId, shineId }: ShapeProps) {
  return (
    <g>
      {/* Left handle */}
      <path
      ...
      {/* Stem */}
      <rect x="22" y="30" width="4" height="7" fill={`url(#${gradId})`} stroke={GRADIENT_STOPS.gold.lo} strokeWidth="0.4" />
      {/* Base */}
      <path
...
function DiamondShape({ gradId, shineId }: ShapeProps) {
  return (
    <g>
      {/* Top facets */}
      <path d="M14 12 L24 6 L34 12 L30 14 L24 11 L18 14 Z" fill={`url(#${gradId})`} stroke={GRADIENT_STOPS.diamond.lo} strokeWidth="0.5" />
      {/* Crown */}
      <path d="M18 14 L24 11 L30 14 L32 18 L24 16 L16 18 Z" fill={`url(#${gradId})`} stroke={GRADIENT_STOPS.diamond.lo} strokeWidth="0.5" />
      {/* Pavilion (bottom) */}
      <path d="M16 18 L24 16 L32 18 L24 42 Z" fill={`url(#${gradId})`} stroke={GRADIENT_STOPS.diamond.lo} strokeWidth="0.6" />
      {/* Facet lines */}
