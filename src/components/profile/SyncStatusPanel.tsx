...
export function SyncStatusPanel({ syncing, online, onSyncNow, onLogin }: SyncStatusPanelProps) {
  ...
  if (!isSupabaseConfigured) {
    return (
      <NestedStat
        value={pl.accountLocalOnly}
        hint={pl.syncFaqLocal}
      />
    )
  }

  if (!snapshot) {
    return <NestedStat value={pl.loading} />
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <details className="group rounded-[var(--sr-radius-md)] border border-[var(--sr-border-subtle)] bg-[var(--sr-bg-surface)]/60">
        <summary
          className={cn(
