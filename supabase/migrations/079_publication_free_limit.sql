-- 079_publication_free_limit.sql
-- Server-side free-tier cap on community publications: max 3 active
-- ('published') plans per author for free users. Pro / trial / lifetime are
-- unlimited. The client gates the publish UI earlier (Plans.tsx), this RPC
-- check is the enforcement layer — it cannot be bypassed by calling the RPC
-- directly.
--
-- CREATE OR REPLACE of publish_community_plan from 035, preserving ALL
-- validations (slug, snapshot, display_name, tags, public profile, rate
-- limit, row-lock idempotency). Only the publication-cap block is new.
